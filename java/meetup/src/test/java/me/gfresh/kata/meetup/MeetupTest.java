...
public class MeetupTest {
    ...
    @Test
    public void test_custom_schedule() {
        Meetup meetup = new Meetup(4, 2015);
        MeetupSchedule schedule = (month, year, day) -> {
            DateTime candidate = MeetupSchedule.FIRST.calculate(month, year, day);
            return isOfficialHoliday(candidate) ? candidate.plusDays(1) : candidate;
        };
        DateTime actual = meetup.day(MONDAY, schedule);
        DateTime expected = new DateTime(2015, 4, 7, 0, 0);
        assertThat(actual).isEqualTo(expected);
    }

    private boolean isOfficialHoliday(DateTime date) {
        return true; // Just for indicating a possible use-case.
    }
}
